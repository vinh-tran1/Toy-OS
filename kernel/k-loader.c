...
//STEP 6 HELPER, I PASTED FROM KERNEL.C
...
static int program_load_segment(proc* p, const elf_program* ph,
    ...
    for (uintptr_t addr = va; addr < end_mem; addr += PAGESIZE) {
       freepage = findFreePage(p->p_pid);
        if (/*assign_physical_page(addr, p->p_pid) < 0
            ||*/ virtual_memory_map(p->p_pagetable, addr, freepage, PAGESIZE,
                                  PTE_P | PTE_W | PTE_U) < 0) {
    ...
    if ((ph->p_flags & ELF_PFLAG_WRITE) == 0)
    {
        ...
        {
            vmap = virtual_memory_lookup(p->p_pagetable, i);
            virtual_memory_map(p->p_pagetable, i, vmap.pa, PAGESIZE, PTE_P | PTE_U);
        }
    }
