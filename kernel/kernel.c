...
// My own helper function declaration for step 2
//-----------------------------------
uintptr_t findFreePage(pid_t pid);
x86_64_pagetable *linkFreePages (pid_t pid);
int numFreePages();
//-----------------------------------

//My own helper function declaration for step 7
//-----------------------------------
void free_helper(int pn);
void free_mapped_pages(pid_t pid);
...
void kernel(const char* command) {
    ...
    //check for in kernel code
    virtual_memory_map(kernel_pagetable, (uintptr_t)0,
        (uintptr_t)0, PROC_START_ADDR, PTE_P | PTE_W);
        //why not KERNEL_START_ADDR

    //console should be writeable
    virtual_memory_map(kernel_pagetable, CONSOLE_ADDR,
        CONSOLE_ADDR, PAGESIZE, PTE_P | PTE_W | PTE_U);
...
{
    int count = 0;
    for (uintptr_t i = 0; i < MEMSIZE_PHYSICAL; i+= PAGESIZE)
        if (pageinfo[PAGENUMBER(i)].refcount == 0)
        {
            count ++;
        }
    return count;
...
uintptr_t findFreePage (pid_t pid)
{
    for (uintptr_t i = 0; i < MEMSIZE_PHYSICAL; i+= PAGESIZE)
        if (pageinfo[PAGENUMBER(i)].refcount == 0)
        {
            if (assign_physical_page(i, pid) == -1)
                return (uintptr_t)-1;
            else  
                return i;
        }
...
x86_64_pagetable *linkFreePages (pid_t pid)
{
    x86_64_pagetable *proc_pagetables[5]; vamapping vmap;

    for (int i = 0; i < 5; i++)
    {
        if ((proc_pagetables[i] = ((x86_64_pagetable *)findFreePage(pid))) == (x86_64_pagetable *)-1)
            return NULL;
        memset(proc_pagetables[i], 0, PAGESIZE);
    }
    ...
    {
        vmap = virtual_memory_lookup(kernel_pagetable, i);
        virtual_memory_map(proc_pagetables[0], i, vmap.pa, PAGESIZE, vmap.perm);
    }
...
void process_setup(pid_t pid, int program_number) {
    ...
    processes[pid].p_pagetable = linkFreePages(pid);
    //----------------
    ...
    uintptr_t stack_page = findFreePage(pid);
...
void free_mapped_pages(pid_t pid)
{
    ...
    {
        //vmap = virtual_memory_lookup(current->p_pagetable, addr);
        vmap = virtual_memory_lookup(processes[pid].p_pagetable, addr);
        free_helper(vmap.pn);
    }
}
...
void free_pagetable(x86_64_pagetable* pt)
{
    int pn;
     //---------free page table-------
    //this loads each level of page table into an array so I have an references to every page
    x86_64_pagetable* temp_pagetable[5];

    //temp_pagetable[0] = current->p_pagetable;
    temp_pagetable[0] = pt;
    temp_pagetable[1] = (x86_64_pagetable*)PTE_ADDR(temp_pagetable[0]->entry[0]);
    temp_pagetable[2] = (x86_64_pagetable*)PTE_ADDR(temp_pagetable[1]->entry[0]);
    temp_pagetable[3] = (x86_64_pagetable*)PTE_ADDR(temp_pagetable[2]->entry[0]);
    temp_pagetable[4] = (x86_64_pagetable*)PTE_ADDR(temp_pagetable[2]->entry[1]);

    //loop thorugh array and free each level
    for (int i = 0; i < 5; i++)
    {
        pn = PAGENUMBER(temp_pagetable[i]);
        pageinfo[pn].refcount = 0;
        pageinfo[pn].owner = PO_FREE;
    }
    //free_helper(PAGENUMBER(temp_pagetable[i]));
}
...
void exception(x86_64_registers* reg) {
    ...
    case INT_SYS_PAGE_ALLOC: {
        ...
        //checks to see if address is not in kernel code and not CGA!
        //--------------------------------------------------//
        if (addr % PAGESIZE != 0 || addr >= MEMSIZE_VIRTUAL || addr < PROC_START_ADDR)
        {
        ...
        int r = findFreePage(current->p_pid);
        if ((int)r >= 0)
            virtual_memory_map(current->p_pagetable, addr, r,
                               PAGESIZE, PTE_P | PTE_W | PTE_U);
        else  
            r = -1;
        ...
        current->p_registers.reg_rax = r >= 0 ? 0 : -1;
    ...
    case INT_SYS_FORK:
    {
        pid_t child_pid; int rax = 0;
        //finds free process, makes a pagetable for it
        int i;
        for (i = 1; i < NPROC; i++)
        {
            if (processes[i].p_state == P_FREE)
            {
                ...
                processes[i].p_pagetable = linkFreePages(processes[i].p_pid);
                break;
            }      
        }
        ...
        else
        {
            ...
            //copy processes pagetable to the one I just made
            vamapping vmap; uintptr_t freepage;
            for (uintptr_t j = PROC_START_ADDR; j < MEMSIZE_VIRTUAL; j+= PAGESIZE)
            {
                //get current process virtual memory 
                vmap = virtual_memory_lookup(current->p_pagetable, j);

                //check if it's writeable, then copy the page
                if ((vmap.perm & PTE_W) && (vmap.perm & PTE_U) && (vmap.perm & PTE_P))
                {
                    freepage = findFreePage(child_pid);

                    //check if no fre page i.e. fork fails
                    if ((int)freepage < 0)
                    {
                        free_mapped_pages(child_pid);
                        free_pagetable(processes[child_pid].p_pagetable);
                        processes[i].p_state = P_FREE;
                        rax = -1;
                        current->p_registers.reg_rax = -1;
                        break;
                    ...
                    {
                        memcpy((x86_64_pagetable*)freepage, (x86_64_pagetable*)vmap.pa, PAGESIZE);
                        virtual_memory_map(processes[i].p_pagetable, j, freepage, PAGESIZE, vmap.perm);
                    }
                }

                //if not writeable, don't copy, just put into pagetable
                else if ((vmap.perm & PTE_U) && (vmap.perm & PTE_P))
                {
                    pageinfo[PAGENUMBER(vmap.pa)].refcount++;
                    virtual_memory_map(processes[i].p_pagetable, j, vmap.pa, PAGESIZE, vmap.perm);
                }
            }

            if (rax == 0)
            {
                processes[i].p_registers = *reg;
                processes[i].p_registers.reg_rax = rax;
    ...
    case INT_SYS_EXIT:
        ...
        //     free_helper(vmap.pn);
        // }
        free_mapped_pages(current->p_pid); //does the above code
        ...
        // for (int i = 0; i < 5; i++)
        //     free_helper(PAGENUMBER(temp_pagetable[i]));
        free_pagetable(current->p_pagetable); //does above code
